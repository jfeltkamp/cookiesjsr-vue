...
<template>
  <li :class="['cookiesjsr-service-group', (id === activeGroup) ? 'active': '']">
    <div class="cookiesjsr-service-group--tab"
         @click.prevent="() => setActiveGroup(id)">{{ $t(id + '.title') }}</div>
    <div class="cookiesjsr-service-group--content">
      <div class="cookiesjsr-service-group--intro" v-trans>{{ $t(id + '.title') }}</div>
      <ul class="cookiesjsr-service-group--services">
        <GroupConsent v-if="groupConsent" :key="id" :gid="id" :title="$t(id + '.title')" />
